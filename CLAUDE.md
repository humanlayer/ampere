...
- **TypeScript 7** (`typescript@7`, the native tsgo compiler) with the `@effect/tsgo` Oxlint patch. `bun install` runs `effect-tsgo patch --oxlint`, which embeds Effect diagnostics into oxlint's type-aware `effecttsgo` plugin. Do not remove the `prepare` script.
