...
export type { ProcessEventError, ProcessEventHooks, ProcessEventResult } from './internal/transition.js'
 ...
 */
export class ActorScope extends Context.Service<ActorScope, Scope.Scope>()('effect-machine/actor/ActorScope') {}
...
>(
	...
	const call = Effect.fn('effect-machine.actor.call')(function* (event: E) {
		...
		const reply = yield* Deferred.make<ProcessEventResult<{ readonly _tag: string }>, ActorStoppedError>()
		...
		return (yield* Deferred.await(reply).pipe(
			Effect.ensuring(Effect.sync(() => pendingReplies.delete(reply as Deferred.Deferred<unknown, unknown>))),
	...
	return {
		...
		transitions: transitionsPubSub !== undefined ? Stream.fromPubSub(transitionsPubSub) : Stream.empty,
		waitFor,
		awaitFinal,
		sendAndWait,
		subscribe: (fn) => {
...
const buildInspectionHooks = <S extends { readonly _tag: string }, E extends { readonly _tag: string }>(
	actorId: string,
 ...
 * @internal
 */
const runSupervisionLoop = <S extends { readonly _tag: string }, E extends { readonly _tag: string }>(params: {
	...
	Effect.gen(function* () {
		...
		while (true) {
			...
			if (params.supervision.shouldRestart !== undefined && !params.supervision.shouldRestart(generationExit)) {
				yield* Deferred.succeed(params.terminalExitDeferred, generationExit)
				return
			}
...
>(
	...
	const machineServices = yield* Effect.context<R>()
	...
	}).pipe(Effect.withSpan('effect-machine.actor.start'), Effect.provide(machineServices), Effect.asVoid)
...
export const settlePendingReplies = (pendingReplies: Set<Deferred.Deferred<unknown, unknown>>, actorId: string) =>
	Effect.sync(() => {
...
const make = Effect.fn('effect-machine.actorSystem.make')(function* () {
	...
	>(
		...
		withSpawnGate(spawnRegular(id, machine, options)) as Effect.Effect<ActorRef<S, E>, DuplicateActorError, R>
