...
import type { TaggedOrConstructor, BrandedState, BrandedEvent, ExtractReply } from './internal/brands.js'
import { emitWithTimestamp } from './internal/inspection.js'
import { invalidateIndex, resolveTransition, runTransitionHandler, shouldPostpone } from './internal/transition.js'
...
export const materializeMachine = <S, E, R, SD extends SlotsDef>(
	...
	if (handlers === undefined) {
		...
		if (machine._slotsSchema !== undefined && Object.keys(machine._slotsSchema.definitions).length > 0) {
	...
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	const result = new Machine<S, E, R, any, any, SD>(
		machine.initial,
		machine.stateSchema,
		machine.eventSchema,
...
> {
	...
	constructor(
		...
		this._slots = this._slotsSchema !== undefined ? this._slotsSchema._createSlots(resolve) : ({} as SlotCalls<SD>)
	}
	...
	from<NS extends ReadonlyArray<TaggedOrConstructor<VariantsUnion<_SD> & BrandedState>>, R1>(
		...
		build: (
			scope: TransitionScope<
				...
				_ED,
				SD,
				NS[number] extends TaggedOrConstructor<infer S extends VariantsUnion<_SD> & BrandedState> ? S : never
			>,
		) => R1,
	): Machine<State, Event, R, _SD, _ED, SD>
	from(
		...
		build: (scope: TransitionScope<State, Event, R, _SD, _ED, SD, VariantsUnion<_SD> & BrandedState>) => unknown,
	) {
	...
	>(
		...
		for (const state of states) {
			this.addTransition(state, event, handler as TransitionHandler<NS, NE, BrandedState, SD, never>, reenter)
		}
		return this
	}
	...
	spawn<NS extends VariantsUnion<_SD> & BrandedState, R1>(
		state: TaggedOrConstructor<NS>,
		handler: StateEffectHandler<NS, VariantsUnion<_ED> & BrandedEvent, SD, R1>,
	): Machine<State, Event, R | Exclude<R1, Scope.Scope>, _SD, _ED, SD>
	/** Multiple states */
	...
	>(
		state: TaggedOrConstructor<NS>,
		run: (ctx: StateHandlerContext<NS, VariantsUnion<_ED> & BrandedEvent, SD>) => Effect.Effect<A, E1, R1>,
	...
	task(stateOrStates: any, run: any, options: any): Machine<State, Event, R, _SD, _ED, SD> {
		const handler = Effect.fn('effect-machine.task')(function* (
			...
			if (Exit.isSuccess(exit)) {
				...
				const successEvent = options.onSuccess !== undefined ? options.onSuccess(exit.value, ctx) : exit.value
	 ...
	 */
	background<R1>(
		handler: StateEffectHandler<State, Event, SD, R1>,
	): Machine<State, Event, R | Exclude<R1, Scope.Scope>, _SD, _ED, SD> {
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
> {
	...
	reenter<NE extends VariantsUnion<_ED> & BrandedEvent, RS extends VariantsUnion<_SD> & BrandedState>(
		event: TaggedOrConstructor<NE>,
		handler: TransitionHandler<SelectedState, NE, RS, SD, never, ExtractReply<NE>>,
	): TransitionScope<State, Event, R, _SD, _ED, SD, SelectedState> {
 ...
 * ```
 */
export const scoped = <A, E, R>(effect: Effect.Effect<A, E, R>): Effect.Effect<A, E, R | Scope.Scope> =>
	Effect.flatMap(Effect.service(Scope.Scope), (scope) => Effect.provideService(effect, ActorScope, scope))
