...
export type VariantsUnion<D extends Record<string, Schema.Struct.Fields>> = {
	[K in keyof D & string]: TaggedStructType<K, D[K]> &
		(D[K] extends { readonly [ReplySchemaSymbol]: Schema.Schema<infer R> } ? ReplyTypeBrand<R> : unknown)
}[keyof D & string]
 ...
 * ```
 */
export const State = <const D extends Record<string, Schema.Struct.Fields>>(definition: D): MachineStateSchema<D> =>
 ...
 * ```
 */
const EventImpl = <const D extends Record<string, Schema.Struct.Fields>>(definition: D): MachineEventSchema<D> =>
