...
} = <F extends Fields, Return = void>(fields: F, returnSchema?: Schema.Schema<Return>): SlotFnDef<F, Return> => {
...
export interface SlotsSchema<D extends SlotsDef> {
	...
	readonly _createSlots: (
		resolve: <N extends keyof D & string>(name: N, params: SlotParams<D[N]>) => Effect.Effect<SlotReturn<D[N]>>,
	) => SlotCalls<D>
}
 ...
 */
export type SlotHandler<Params, Return, R = never> = (params: Params) => Return | Effect.Effect<Return, never, R>
...
export class MachineContextTag extends Context.Service<MachineContextTag, MachineContext<any, any, any>>()(
...
export const define = <D extends SlotsDef>(definitions: D): SlotsSchema<D> => {
	...
	for (const name of names) {
		...
		requestSchemas.push(Schema.TaggedStruct('SlotRequest', { name: Schema.Literal(name), params: def.inputSchema }))
		resultSchemas.push(Schema.TaggedStruct('SlotResult', { name: Schema.Literal(name), result: def.outputSchema }))
 ...
 * ```
 */
const of = <D extends SlotsDef>(slotsSchema: SlotsSchema<D>, provided: ProvideSlots<D>): SlotCalls<D> => {
	const slots: Record<string, unknown> = {}
	for (const name of Object.keys(slotsSchema.definitions)) {
		...
		const call = (params: unknown): Effect.Effect<unknown, never> =>
			Effect.suspend((): Effect.Effect<unknown, never> => {
				...
				return Effect.isEffect(result) ? (result as Effect.Effect<unknown, never>) : Effect.succeed(result)
			})
