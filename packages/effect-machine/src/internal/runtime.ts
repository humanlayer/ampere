 ...
 * @internal
 */
import { Cause, Deferred, Effect, Exit, Fiber, Match, Queue, Ref, Schema, Scope, SubscriptionRef } from 'effect'
...
>(
	...
	const stateRef = config.cellResources?.stateRef ?? (yield* SubscriptionRef.make<S>(machine.initial))
	const stoppedRef = config.cellResources?.stoppedRef ?? (yield* Ref.make(false))
	const eventQueue =
		config.cellResources?.eventQueue ?? (yield* config.queueFactory ?? Queue.unbounded<RuntimeQueuedEvent<E>>())
	...
	const defaultSpawn: MachineRef<E>['spawn'] = (childId, childMachine) =>
		system.spawn(`${childPrefix}${childId}`, childMachine).pipe(Effect.provideService(ActorSystemTag, system))
		...
		spawn:
			onChildSpawned !== undefined
				? (childId, childMachine) =>
						defaultSpawn(childId, childMachine).pipe(Effect.tap((child) => onChildSpawned(childId, child)))
				: defaultSpawn,
		reply: (value: unknown) =>
			Effect.sync(() => {
...
>(
	...
	const processQueued = Effect.fn('effect-machine.runtime.processQueued')(function* (queued: EventQueued) {
		...
		yield* Match.value(queued).pipe(
			Match.tagsExhaustive({
				call: (call) => Deferred.succeed(call.reply, result as ProcessEventResult<{ readonly _tag: string }>),
				sendWait: (sendWait) => Deferred.succeed(sendWait.done, undefined),
				ask: (ask) =>
					Effect.gen(function* () {
						if (result.hasReply) {
							...
							if (replySchema !== undefined) {
								const decoded = yield* Schema.decodeUnknownEffect(replySchema)(result.reply).pipe(
									Effect.catch((decodeError) =>
										Effect.gen(function* () {
											yield* Deferred.die(ask.reply, decodeError)
											return yield* Effect.die(decodeError)
										}),
									),
								)
								yield* Deferred.succeed(ask.reply, decoded)
							} else {
								yield* Deferred.succeed(ask.reply, result.reply)
							}
							...
							deferredReplyRef.current = ask.reply
						} else {
							yield* Deferred.fail(ask.reply, new NoReplyError({ actorId, eventTag: event._tag }))
						}
					}),
				send: () => Effect.void,
			}),
		...
		Effect.gen(function* () {
			...
			for (const entry of remaining) {
				if (entry._tag === 'sendWait') {
					...
					forkEffect(Deferred.fail(entry.reply, new NoReplyError({ actorId, eventTag: entry.event._tag })))
	...
	while (true) {
		...
		const { shouldStop, stateChanged } = yield* wrapped.pipe(
			Effect.catchCause((cause) => {
				...
				return shutdown(ActorExit.Defect(cause, phase)).pipe(Effect.andThen(Effect.failCause(cause)))
			}),
		)
