...
>(
	...
	const resolved = isEffect(raw)
		? yield* (raw as Effect.Effect<S | ReplyResult<S, unknown> | DeferReplyResult<S>, never, R>).pipe(
 ...
 * @internal
 */
export const shouldPostpone = <S extends { readonly _tag: string }, E extends { readonly _tag: string }, R>(
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
		...
		const effect = spawnEffect
			...
			.pipe(
				...
				Effect.catchCause((cause) => {
					...
					const report =
						reportError !== undefined ? reportError({ phase: 'spawn', state, event, cause }) : Effect.void
 ...
 */
type TransitionIndex<S, E, SD extends SlotsDef, R> = Map<string, Map<string, Array<Transition<S, E, SD, R>>>>
 ...
 */
const getIndex = <S extends { readonly _tag: string }, E extends { readonly _tag: string }, R, SD extends SlotsDef>(
	// eslint-disable-next-line @typescript-eslint/no-explicit-any -- Schema fields need wide acceptance
	machine: Machine<S, E, R, any, any, SD>,
