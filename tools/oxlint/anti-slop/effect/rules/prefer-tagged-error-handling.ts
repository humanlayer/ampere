...
const BROAD_CATCH_METHODS = new Set(["catch", "catchIf"]);
...
const tagComparison = (node: ESTree.Node): ESTree.MemberExpression | undefined => {
	if (node.type !== "BinaryExpression" || !EQUALITY_OPERATORS.has(node.operator)) return undefined;
...
const isEffectBroadCatch = (node: ESTree.CallExpression): boolean =>
	...
	BROAD_CATCH_METHODS.has(node.callee.property.name);
...
/** Require Effect's tagged catch operators for errors and tagged error reasons. */
export const preferTaggedErrorHandlingRule = defineRule({
	meta: {
		type: "problem",
		docs: {
